...
export default function Projekte() {
  ...
  const [showProjectNav, setShowProjectNav] = useState<boolean>(false);
  ...
  return (
    <section id="portfolio" className="w-full min-h-[calc(100vh-65px)] relative bg-white">
      ...
      <div className="h-[calc(100vh-65px)] w-full relative">
        {currentProject.screenshots && currentProject.screenshots.length > 0 ? (
          <div className="relative h-full w-full bg-gray-50 overflow-hidden">
            ...
            {/* Project Navigation at Bottom Left - Collapsed by default */}
            <div
              className={`absolute bottom-8 left-8 z-20 transition-all duration-300 ease-in-out`}
              onMouseEnter={() => setShowProjectNav(true)}
              onMouseLeave={() => setShowProjectNav(false)}
            >
              <div
                className={`bg-[#3a3a3a90] backdrop-blur-sm rounded-lg p-4 transition-all duration-300 ${
                  showProjectNav ? "opacity-100 scale-100" : "opacity-80 scale-95 hover:opacity-100 hover:scale-100"
                }`}
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  >
                    ...
                    <svg
                      ...
                      className={`h-4 w-4 transition-transform ${showProjectNav ? "rotate-180" : ""}`}
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
                ...
                <div
                  className={`space-y-3 transition-all duration-300 ${
                    showProjectNav ? "max-h-48 opacity-100" : "max-h-0 opacity-0 pointer-events-none"
                  } overflow-hidden`}
                >
                  {projectPins.map((project) => (
                    ...
                    >
                      <div className={`${project.id === activeProject ? "opacity-100" : "opacity-70 hover:opacity-100"}`}>
                        <h4 className={`font-satoshibold text-sm mb-1 ${project.id === activeProject ? "text-[#F65009]" : "text-white"}`}>
                          {project.name}
                        </h4>
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent pb-4 pt-20 px-8 z-10">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {/* Description */}
            <div className="col-span-1 md:col-span-2">
              <h3 className="text-xl font-satoshibold text-white mb-4">Beschreibung</h3>
              <p className="text-white/80">{currentProject.description}</p>
            </div>

            {/* Services */}
            <div>
              <h3 className="text-xl font-satoshibold text-white mb-4">Services</h3>
